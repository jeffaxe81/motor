...
export function buildAssetApp(options: BuildAssetAppOptions) {
  ...
  app.get("/api/v1/assets/:id/relations", async request => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; return service.listRelatedAssets(context, id); });
  app.post("/api/v1/assets/:id/relations", async (request, reply) => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; return reply.status(201).send(await service.linkAsset(context, id, request.body)); });
  app.get("/api/v1/assets/:id/dispatch-references", async request => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; return service.listDispatchReferences(context, id); });
  app.post("/api/v1/assets/:id/dispatch-references", async (request, reply) => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; return reply.status(201).send(await service.linkDispatchReference(context, id, request.body)); });
  app.get("/api/v1/assets/:id/telemetry", async request => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; return service.listTelemetry(context, id); });
  app.post("/api/v1/assets/:id/telemetry", async (request, reply) => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; return reply.status(201).send(await service.recordTelemetry(context, id, request.body)); });
