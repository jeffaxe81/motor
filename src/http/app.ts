...
export function buildAssetApp(options: BuildAssetAppOptions) {
  ...
  app.post("/api/v1/assets/:id/evidence", async (request, reply) => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; const evidence = await service.addEvidence(context, id, request.body); return reply.status(201).send(evidence); });
  ...
  app.post("/api/v1/assets/:id/inspections", async (request, reply) => { const context = await options.resolveContext(request); const { id } = request.params as { id: string }; const inspection = await service.recordInspection(context, id, request.body); return reply.status(201).send(inspection); });
