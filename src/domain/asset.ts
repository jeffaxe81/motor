...
const maintenancePartSchema = z.object({
  ...
  unitCost: z.number().finite().nonnegative().max(1000000000),
}).strict();
...
const maintenanceWarrantySchema = z.object({
  reference: z.string().trim().min(1).max(300),
  validUntil: z.string().date(),
  documentReference: z.string().trim().min(1).max(500).optional(),
}).strict();

const maintenanceLinksSchema = z.object({
  inspectionId: z.string().trim().min(1).max(300).optional(),
  orderReference: z.string().trim().min(1).max(300).optional(),
  eventReference: z.string().trim().min(1).max(300).optional(),
}).strict();

export const assetMaintenanceInputSchema = z.object({
  kind: z.string().trim().min(1).max(100),
  description: z.string().trim().min(1).max(500),
  parts: z.array(maintenancePartSchema).max(200).default([]),
  costs: z.array(maintenanceCostSchema).max(200).default([]),
  warranty: maintenanceWarrantySchema.optional(),
  links: maintenanceLinksSchema.optional(),
